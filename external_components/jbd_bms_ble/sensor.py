...
from esphome.const import (
    CONF_ID, DEVICE_CLASS_VOLTAGE, DEVICE_CLASS_CURRENT, 
...
CONFIG_SCHEMA = cv.Schema({
    ...
    cv.Optional("temperature"): sensor.sensor_schema(unit_of_measurement=UNIT_CELSIUS, device_class=DEVICE_CLASS_TEMPERATURE, accuracy_decimals=1),
...
async def to_code(config):
    ...
    for key in ["voltage", "current", "temperature", "soc", "cycles"]:
        if key in config:
